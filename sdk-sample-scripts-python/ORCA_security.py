...
## ORCA API URL
ORCA_URL = "https://app.au.orcasecurity.io/api"

## ORCA Bearer Token (can also use os.environ variable)
TOKEN = ""
...
def orca_api(query):
    ...
    response = requests.get(url=ORCA_URL, headers=HEADERS, params=query)
...
def get_devices():
    ...
    ## ORCA query parameters when making API call
    query = {
      "limit": "500",
      "page": "0"
    }
...
def import_data_to_runzero(assets: list[ImportAsset]):
    ...
    my_asset_source = custom_source_mgr.get(name="ORCA")
    if my_asset_source:
        source_id = my_asset_source.id
    else:
        my_asset_source = custom_source_mgr.create(name="ORCA")
        source_id = my_asset_source.id
    ...
    import_task = import_mgr.upload_assets(
        ...
        assets=assets,
        task_info=ImportTask(name="ORCA Sync"),
    )
