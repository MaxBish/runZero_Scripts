...
CBC_hostname = '' ## CarbonBlack hostname
CBC_ORG_KEY = '' ## CarbonBlack Org Key
CARBON_BLACK_API_URL = 'https://{}/appservices/v6/orgs/{}/devices/_scroll'.format(CBC_hostname,CBC_ORG_KEY)
HEADERS = 'CBC API KEY' ## CarbonBlack API Key/Auth Token
...
def get_assets():
    ...
    while True:
        response = requests.get(CARBON_BLACK_API_URL,params=params,headers=HEADERS)
        ...
        for record in response:
            rz_import_assets.append(record)

    print("Got a list of " + str(rz_import_assets) + " Devices from Carbon Black API")

    for device in rz_import_assets:
        custom_attrs = {}
        custom_attrs['os_version'] = device['criteria']['os_version']
        custom_attrs['os'] = device['criteria']['os']

        mac_address = None
        if device.mac_address is not None:
            mac_address = device['criteria']['mac_address']

        ips = []
        ips.append(device['criteria']['last_internal_ip_address'])
